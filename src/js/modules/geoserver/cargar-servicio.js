...
// inicio ortofoto
var orotofoLayer = new LayerTile({
  title: 'Global Imagery',
  source: new SourceTileWMS({
    url: serverConfig.ortofoto.url,
    params: {
      'FORMAT': serverConfig.ortofoto.format,
      'VERSION': '1.1.1',
      tiled: true,
      STYLES: '',
      LAYERS: serverConfig.ortofoto.layers,
    }
  })
});

window.map.addLayer(orotofoLayer);
// fin ortofoto
...
// inicia capa poligonos de muestreo
var serviceSource3 = new SourceVector({
  loader: function(extent, resolution, projection) {
    ...
    var url = serverConfig.url_poligonos_muestreo + '&srsname=EPSG:3857&bbox=' + extent.join(',') + ',EPSG:3857'
    // use jsonp: false to prevent jQuery from adding the "callback"
    // parameter to the URL
    $.ajax({url: url, dataType: 'json', jsonp: false}).done(function(response) {
      serviceSource3.addFeatures(geojsonFormat.readFeatures(response))
    });
  },
  strategy: Loadingstrategy.tile(Tilegrid.createXYZ({maxZoom: 19}))
});

var serviceLayer3 = new LayerVector({
  source: serviceSource3,
...
window.map.addLayer(serviceLayer3);

// termina capa poligonos de muestreo

//inicia segunda capa poligono
...
// termina capa puntos datos


//style1
var image = new Circle({
  radius: 5,
  fill: new Fill({color: 'rgba(223, 62, 62, 1)'}),
  stroke: new Stroke({color: 'rgba(116, 43, 8, 0.45)', width: 1})
});
...
var styles2 = {
  ...
  'MultiPoint': new Style({image: image}),
  'MultiPolygon': new Style({
    stroke: new Stroke({color: 'rgb(58, 153, 0)', width: 1}),
    fill: new Fill({color: 'rgba(43, 255, 0, 0.1)'})
  }),
  'Polygon': new Style({
    stroke: new Stroke({color: 'blue', lineDash: [4], width: 3}),
    fill: new Fill({color: 'rgba(0, 0, 255, 0.1)'})
...
var styles3 = {
  ...
  'MultiPoint': new Style({image: image}),
  'MultiPolygon': new Style({
    stroke: new Stroke({color: 'rgb(153, 0, 0)', width: 1}),
